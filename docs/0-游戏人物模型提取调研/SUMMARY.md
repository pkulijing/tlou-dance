...
4. **Spike #3「面部 mocap」**（P2，可选）：如果想要表情，调研后端是 [Autodesk Flow Studio](https://wonderdynamics.com/) Free tier 还是 SMPL-X / FreeMoCap + VIPER 开源拼装；前端要给角色加 ARKit 52 blendshape（见调研记录）。**建议 Spike #2 通了之后再回来评估**。
