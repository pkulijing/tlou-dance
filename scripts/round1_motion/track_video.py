"""4D-Humans video tracking entrypoint —— 从上游 https://github.com/shubham-goel/4D-Humans/blob/main/track.py
逐字 copy（hmr2 包不带顶层 entry，必须本地落一份才能用 hydra CLI 跑）。

调用例（从 worktree 根跑）::

    uv run python scripts/round1_motion/track_video.py \\
        video.source=data/round1_motion/clip/friendly_10s.mp4 \\
        video.output_dir=outputs/round1_motion/friendly

权重路径：PHALP 默认从 ~/.cache/phalp/ 读，4D-Humans 默认从 ~/.cache/4DHumans/ 读，
两套独立。第一次跑会触发 PHALP 自动下权重（fbaipublicfiles），国内可能慢。
"""
...
class HMR2023TextureSampler(HMR2Predictor):
    ...
    def forward(self, x):
        ...
        def unproject_uvmap_to_mesh(bmap, fmap, verts, faces):
            ...
            bs = face_verts.shape  # noqa: F841 (上游 4D-Humans/track.py 原样保留)
...
class Human4DConfig(FullConfig):
    ...
    expand_bbox_shape: tuple[int] | None = (192, 256)
    pass
